...
public partial class ManageESPBankAccounts
{
    ...
    protected override async Task OnAuthorizedInitAsync()
    {
        ...
        else if (string.Equals(Action, "edit", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (result.Success)
            {
                ...
                await SessionStorage.DeleteAsync("banAccountEditIntent");
            }
        }
    }

    private static readonly string LoadError = "Unable to load bank accounts. Please try again.";
    ...
    private async Task LoadAccountsAsync()
    {
        ...
        try
        {
            _accounts = await ESPBankAccountOrchestrator.GetExistingAccountsAsync();
    ...
    private async Task ConfirmRemove()
    {
        if (_accountToRemove != null)
        {
            try
            {
                var result = await ESPBankAccountOrchestrator.InactivateBankAccountAsync(_accountToRemove.BankAccountSk);
