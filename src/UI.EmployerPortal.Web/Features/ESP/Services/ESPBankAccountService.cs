...
internal class ESPBankAccountService : IESPBankAccountService
{
    ...
    private readonly IAsyncRetryPolicy<ESPBankAccountService> _retryPolicy;

    public ESPBankAccountService(
        IEFTPaymentService eftPaymentService,
        IESPService eSPService,
        IUserAccountService userAccountService,
        IAsyncRetryPolicy<ESPBankAccountService> retryPolicy)
    {
    ...
    public async Task<SaveBankAccountResult> InactivateBankAccountAsync(int bankAccountSk, int employerAccountSk)
    {
        try
        {
            ...
            var response = await _retryPolicy.ExecuteAsync(() =>
            {
                return _espService.InactivateBankAccountAsync(request);
            });
    ...
    public async Task<SaveBankAccountResult> SaveBankAccountAsync(BankAccountModel model)
    {
        try
        {
            ...
            var request = new ESPBankAccountRequest
            {
            ...
            var response = await _retryPolicy.ExecuteAsync(() =>
            {
                return _espService.SaveBankAccountAsync(request);
            });
    ...
    public async Task<IReadOnlyList<SavedBankAccount>> GetExistingAccountsAsync()
    {
        try
        {
            ...
            var response = await _retryPolicy.ExecuteAsync(() =>
            {
                return _espService.LoadServiceProviderBankAccountsAsync(secureUserSk);
            });
    ...
    public async Task<BankAccountModel?> GetBankAccountForEditAsync(int bankAccountSk)
    {
        try
        {
            ...
            var request = new ObtainESPBankAccountRequest
            {
                SecureUserSK = secureUserSk,
                BankAccountSK = bankAccountSk,
            };
            ...
            var response = await _retryPolicy.ExecuteAsync(() =>
            {
                return _espService.ObtainBankAccountAsync(request);
            });
    ...
    public async Task<IReadOnlyList<PendingPayment>> GetPendingPaymentsAsync(int bankAccountSk)
    {
        try
        {
            ...
            return response.BankAccounts.FirstOrDefault(bankAccount =>
            {
                return bankAccount.BankAccountSK == bankAccountSk;
            })?.PendingPayments.Select(pendingPayment =>
            {
                return new PendingPayment
                {
                    SettlementDate = pendingPayment.SettlementDate ?? DateTime.MinValue,
                    ConfirmationNumber = pendingPayment.ConfirmationID ?? string.Empty,
                    Amount = pendingPayment.Amount ?? 0m
                };
            }).ToList() ?? [];
        }
        catch (CommunicationException)
